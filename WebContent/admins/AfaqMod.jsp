	...
	<div class="main-panel">
		<div class="content">
			<div class="container-fluid">
				<h4 class="page-title">FAQ 글 수정</h4>

				<form action="${contextPath}/admin/AfaqModpro.do?faq_num=${faqUpdate.faq_num}" method="POST">
					<div class="form-group">
						<label for="title">Title:</label> <input type="text"
							class="form-control" placeholder="title" id="title" name="title" value="${faqUpdate.faq_title}" required>
					</div>
					<div class="form-group">
						<label for="title">Category:</label>
						<c:if test="${faqUpdate.faq_cate == '취소/환불' }">
							<select class="form-control input-square" id="squareSelect" name= "cate" style="width: 10%;">
							<option value="취소/환불" selected="selected">취소/환불</option>
							<option value="이벤트">이벤트</option>
							<option value="티켓수령">티켓수령</option>
							<option value="회원">회원</option>
							</select>
						</c:if>	
						<c:if test="${faqUpdate.faq_cate == '이벤트' }">
							<select class="form-control input-square" id="squareSelect" name= "cate" style="width: 10%;">
						...
						<c:if test="${faqUpdate.faq_cate == '티켓수령' }">
							<select class="form-control input-square" id="squareSelect" name= "cate" style="width: 10%;">
							<option value="취소/환불">취소/환불</option>
							<option value="이벤트">이벤트</option>
							<option value="티켓수령" selected="selected">티켓수령</option>
							<option value="회원">회원</option>
							</select>
						</c:if>
						<c:if test="${faqUpdate.faq_cate == '회원' }">
							<select class="form-control input-square" id="squareSelect" name= "cate" style="width: 10%;">
							<option value="취소/환불">취소/환불</option>
							<option value="이벤트">이벤트</option>
							<option value="티켓수령">티켓수령</option>
					...
					<div class="form-group">
						...
						<textarea id="summernote" class="form-control" rows="5" name="contents" required >${faqUpdate.faq_contents}</textarea>
					</div>
					<div align="center">
					<button type="submit" class="btn btn-success" >글 수정하기</button>
					<button type="reset" class="btn btn-warning">다시 쓰기</button>
					<button onclick="location.href='${contextPath}/admin/AfaqMain.do'" type="reset" class="btn btn-danger">목록 으로</button>
					</div>
				</form>
